	...
	object target;
		...
		if (! mapp(def = query_emote(verb)))
                ...
                if (stringp(str = def["others_target"]))
                {
                        ...
                        str = replace_string(str, "$N", sprintf("%s(%s)", my_name, capitalize(me->query("id"))));
                        ...
                        str = replace_string(str, "$C", RANK_D->query_self_close(rwho,me));
                        //str = replace_string(str, "$c", RANK_D->query_close(rwho["gender"], me->query("age"), rwho["age"]));
                        str = replace_string(str, "$c", RANK_D->query_close(rwho,me));
