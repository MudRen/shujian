...
void create()
{
        set_name(MAG"邪恶纹章"NOR,({"evil emblem","emblem","evil"}));
        set_weight(200);
        if( clonep() )
                set_default_object(__FILE__);
        else {
  
        set("long",HIR"这枚纹章有着强大的魔力。\n"NOR);
        set("unit","枚");
        set("value",100000);
        seteuid(getuid());
        set("no_get",1);
...
void init()
{
		...
		add_action("do_full","full");//full气血		
		wear();        
        }
...
int unequip()
{
	...
	if (ob && userp(ob) && !wizardp(this_player()))
