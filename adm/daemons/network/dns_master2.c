...
void read_callback(int sock, string msg);
...
int startup_udp()
{
        ...
        socket_id = socket_create(STREAM, "read_callback", "close_callback");
        if (socket_id < 0) {
        ...
        err_no = socket_listen(socket_id, "listen_back");
        if( err_no <= 0 ) {
                log( sprintf("Failed to listen socket of TCP services, error = %d.\n", err_no));
                socket_close(socket_id);
                return 0;
        }
        return 1;
...
//	message("wizard:yuj", sprintf("有连接来自 %s\n", socket_address(new_fd)), users());
//	if (new_fd < 0) return;
}
...
{
	message("wizard:yuj", sprintf("响应 %s\n", socket_address(fd)), users());
}
*/
...
void read_callback(int sock, string msg)
{
	string func, rest, *bits, name, arg, *list, addr = socket_address(sock), str;
	mapping args;
	int i, flag = 0;

//	message("wizard:yuj", sprintf("read_back %d %s %s\n", sock, socket_address(sock), msg), users());
	...
	if( !sscanf(msg, "@@@%s||%s@@@%*s", func, rest)) {
        ...
        while( i-- ) {
                ...
                // add by aln for disable intermud who not define on /include/net/config.h
...
void send_udp(string host, int port, string msg)
{
	...
	if (socket_connect(sock, host+" "+port, "in_read_callback", "in_write_callback") < 0) {
		socket_close(sock);
		log("Failed connect to " + host + " " + port + "\n");
		return;
	}
//	message("wizard:yuj", sprintf("连接到 %d %s\n", sock, socket_address(sock)), users());
	set("write"+sock, msg);
}

void in_read_callback(int fd, string msg)
{
	message("wizard:yuj", sprintf("有响应来自 %s\n", socket_address(fd)), users());
}
...
//	message("wizard:yuj", sprintf("写完毕 %d %s\n", fd, socket_address(fd)), users());
	while (1) {
...
{
	if (ACCESS_CHECK(previous_object()))
        return muds + ([ mud_nname():this_host + ([ "TIME":""+uptime() ]) +
