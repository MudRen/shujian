...
#ifdef DEBUG
	message("wizard",sprintf("向%s %s 发送emote请求。\n",rhost["HOSTADDRESS"],rhost["PORTUDP"]),users() ); 
#endif
...
#ifdef DEBUG
	message("wizard",sprintf("向%s %s 发送emote回复。\n",rhost["HOSTADDRESS"],rhost["PORTUDP"]),users() ); 
#endif
		DNS_MASTER->send_udp(rhost["HOSTADDRESS"], rhost["PORTUDP"],
			"@@@" + "remote_a" +
			"||NAME:" + Mud_name() +
			...
			Mud_name(), ob->name(1), ob->query("id"), ob->query("age")*12+ob->query("month"), ob->query("gender"),
