...
int move(mixed dest, int silently)
{
	...
	if( (result = ::move(dest, silently)) ) {
		...
		if (!i || !"adm/daemons/wk"->check_item(this_object(), src, dest)) {
			i = query("imbued");
...
void save_armor(object me)
{
	...
	me->set("worker/"+mp["save_id"], copy( "adm/daemons/wk"->convert_armor2user(mp) ));
	if(userp(me)) me->save();
}
...
void delete_weapon2(object me)
{
	...
	if(stringp(query("owner")) && query("owner")!="" && query("owner")!="0" && query("owner")!=getuid(me))
		"adm/daemons/wk"->deleteweapon(query("owner"),"你的"+name()+"因为被"+me->name()+"损坏而被自动删除，已经不再是你的私有武器。\n",query("armor_mp/save_id") );
}
...
void remove(string euid)
{
	...
	if ( !(int)query_temp("quit_dest") ) {
		...
		//if( query("armor_mp/imbued") > 3 )
			//"adm/daemons/wk"->addto_pool("delete",query("armor_mp/save_id")," ");
		...
		//if( query("armor_mp/imbued") > 3 )
			//"adm/daemons/wk"->addto_pool("update",query("armor_mp/save_id"),sprintf("%d",time()));
	}
