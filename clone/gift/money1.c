...
int do_dig(string arg)
{
       ...
       if( random(30 ) > 25) {
        ...
        if(random(100) == 73) {
                message_vision(HIG"$N突然挖到一封信函。信纸发黄，看起来已经有些年头了。\n"NOR,me);
                message_vision(HIG"$N一读之下，发现竟是江南陆家庄陆展元写给其友人的一封求救信函。。\n"NOR,me);
                message_vision(HIG"信中大意是当年李莫愁、武三通二人被迫答应十年内不跟其夫妇为难，十年期满，必来生事，望其前往驰援等等。。\n"NOR,me);
                CHANNEL_D->do_channel( this_object(), "rumor",sprintf("有人看到%s获得一次解"+HIY+"蛤蟆功"+HIM+"的机会！", me->name(1)));
                log_file("job/PRIZE", sprintf("%8s%s获得蛤蟆功新QUEST触发条件。\n", ctime(time())[4..19],
                me->name(1)+"("+me->query("id")+")")  );
                destruct(this_object());
                return 1;       
