...
void create()
{
        set_name (HIY"神眼"HIW"(年卡专供)"NOR, ({ "shenyan","eye" }));
...
if (!me->query("y-card-vip") && !me->query("buyvip"))  return notify_fail("你还不是年卡贵宾，暂时不能使用这个物品。\n");     
	...
	map = ob->query_skill_map();
		...
		destruct(ob);
        	...
        	tell_object(ob,"隐约中你看到天空有一缕白光在你头上闪过。\n");       
        degree(); 
        return 1;
}
