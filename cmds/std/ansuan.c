...
int main(object me, string arg)
{
	...
	if (env == old_env)
		return notify_fail("直接下 kill 指令好了。\n");
  ...
  if (userp(me) && userp(target)&& me->query("combat_exp")<1200000)
		return notify_fail("对不起,只有大于1.2M才可以暗算别人。\n");
	...
	if (userp(me) && userp(target)) {
		...
		if(target->query("no_pk")) return notify_fail("对方已经金盆洗手，你还是放过他吧。\n");
		if(target->query_temp("user_type") == "worker") return notify_fail(target->name(1)+"只是个工匠，你的心肠也太黑了吧。\n");
	}
...
if (userp(target) && me->query("combat_exp")<1000000) //经验少于1M不许PK，防止大米干扰人家JOB！
                return notify_fail("对不起,只有大于1M才可以PK。\n");
...
if (userp(target) && me->query("combat_exp") > target->query("combat_exp") * 3 / 2 &&  !target->query_condition("killer")) 
 ...
 me->apply_condition("no_quit",20);
	...
	&& target->query("oyf/hamagong") > 1) {
		...
		message_vision(CYN"但见$N将身子一蹲，体内蛤蟆功自然催动，将$n一下子反震回去！\n"NOR, target, me);
		COMBAT_D->do_attack(target, me, target->query_temp("weapon"), 2);
		me->move(old_env);
		write(
			"你这一下迅速退回，虽然并无大碍，但也惊呆了！\n"
			"看来暗算对于拥有蛤蟆功的人来说不怎么起作用！\n"
		);
