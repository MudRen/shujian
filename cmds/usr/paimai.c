...
*/
        ...
        if(sscanf(arg,"%s for %d %s",str,num,unit) == 3)
        {
                ...
                if(ob->query("no_drop") || ob->query("dzd") || ob->query("no_sell") || ob->query("money_id") || ob->is_character() || ob_id == "corpse" || ob_id == "skeleton" || ob_id == "huo ba" || ob_id == "deng long" || ob_id == "la zhu")
                {
        ...
        if(sscanf(arg,"%s add %d %s", ob_id,num,unit) == 3)
        {
                ...
                if(me->query("age") < 18)
                {
                        ...
                        return 1;
                                ...
                                if( num > 500 )
                                {
                                        write("现在限制一次最多加五百两黄金以避免不必要的问题产生\n");
                                        return 1;
                                }
