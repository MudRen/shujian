...
int ask_me(object who)
{
        ...
        object obj;
        int times,gold,gold2,gold3,tb2,exp,tb,exp1,this_time;

        string str = "";
       int i = random(3);
        times = (int)me->query("job_time/官府",1);

        if ( times<49 || times - me->query("job_time/官府领取") <=49 ){
...
this_time=times- me->query("job_time/官府领取");
if( (int)me->query("gf_gives") < (int)me->query("age") && time()- me->query("gf_job_time")>86400 ) 
{
    ...
    gold2=gold/100;
    gold3=gold/200;
    tb = random(this_time /2) + 1; 
    tb2 = random(this_time/8)+1; 
             ...
             if(this_time >= 500 && random(10) > 5 )
{
  ...
  tell_object(me,HIY"你额外获得了" + CHINESE_D->chinese_number(exp) + "点经验奖励！\n"NOR);
                      me->add("combat_exp", exp);
                      me->set_temp("title", HIR"江湖神捕"NOR);


}

 me->set("job_time/官府领取",times);
