...
int do_yanjiu(string arg)
{
	...
	if (me->query("quest/天龙八部/北冥神功/fail") >=3 && me->query("registered") < 3)
		return notify_fail("就你现在的情况，恐怕没有机会领悟无量山中的武功了。\n");
		
		time = time() - me->query("quest/天龙八部/北冥神功/time");
       if ( me->query("quest/天龙八部/北冥神功/time") && time < 86400 ) 
		       return notify_fail("你来的也太勤快了些吧。\n");
/*
if(me->query("quest/天龙八部/北冥神功/combat_exp") && me->query("combat_exp")-me->query("quest/天龙八部/北冥神功/combat_exp")<100000)
