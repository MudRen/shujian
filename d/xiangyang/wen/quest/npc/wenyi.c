...
int ask_xia1()
{
    ...
    if (me->query_temp("quest/金蛇掌法/start",1)) {
...
int accept_object(object who, object ob)
{
    ...
    object me = this_player();
        ...
        if ((string)ob->query("name") != HIY"金蛇锥"NOR || ob->query("id") != "jinshe zhui"){
               command("shake "+ (string)who->query("id"));
               command("say 谢谢你啦。");
               return 0;
        }
        if( !this_player()->query_temp("quest/金蛇剑法/金蛇锥"))
        {
             command("hmm");
