...
void create()
{
        set_name(HIR"金蛇秘籍"NOR, ({ "zhangfa miji", "miji" }));
        set_weight(900);
        if( clonep() )
                set_default_object(__FILE__);
        else {
                set("unit", "本");
                set("long","这是金蛇郎君的武学秘籍，据说可以学到金蛇掌法。\n");
                set("material", "paper");
...
int do_du(string arg)
{
        ...
        if (arg=="jianfa"){       
                ...
                if ( jlevel < 101 ) return notify_fail("你的基础不够,还是学完第一册再说吧\n");
                if( (int)me->query("combat_exp") < (int)jlevel*jlevel*jlevel/10 ) {
                        write("你的实战经验不足，再怎么读也没用。\n");
                        return 1;
                }
                if( me->query_skill("jinshe-jianfa", 1) >= 201){
