...
int du_jiang()
{
        ...
        if(me->query_encumbrance()/100 > me->query("neili") + cost
         ...
         tell_room(environment(me), YEL"，在船上一点，" NOR, ({me}));
         if( !random(50) ){
                me->start_busy(5);
                tell_room(environment(me), YEL"，正要向对岸跃出，突见船老大一竿子扫出，将！"YEL+me->name()+"打落在河中！\n" NOR, ({me}));
                tell_object(me, "你正要继续跃出，突然小腿一疼，“扑通”一声掉进黄河之中！抬起头才发现是船老大用竿子将你扫下船的······\n");
                ...
                me->move(__FILE__);
                message_vision(HIC"$N湿漉漉地从黄河中爬上岸来，叉着腰面对河中渡船叽叽咕咕骂个不停······！\n"NOR,me);
        }

        else {
                me->start_busy(3);
                tell_room(environment(me), YEL"，又向对岸跃出！\n" NOR, ({me}));
                write("你在河中渡船上轻轻一点，又提气飞纵向对岸。\n");
                me->move(SHIP_TO);
