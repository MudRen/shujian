...
nosave nomask protected mixed _query( mapping map, string *parts )
{
	mixed value;
	int i, s;
...
nosave nomask protected int _delete( mapping map, string *parts )
{
	if( sizeof( parts ) == 1 ) {
...
nosave nomask protected mixed _set( mapping map, string *parts, mixed value )
{
	if( sizeof( parts ) == 1 )
