...
void create()
{
        ...
        set_skill("blade", 500);
        set_skill("zhenwu-qijiezhen", 400);
        set_skill("literate", 500);
        set_skill("zhenshan-mianzhang", 600);
...
//		"敌对" : (: ask_no_save :),
                "绕指柔剑" : (: ask_raozhi :),
                "倚天屠龙功" : (: ask_yttlg :),
   //             "往事" : (: ask_wdjiuyang :),    // 武当九阳 by hongba
   //              "九阳神功" : (: ask_wdjiuyang1 :), // 武当九阳正式开始 by hongba
        ]));
