...
if (userp(tar) && userp(me) && me->query("combat_exp")<  tar->query("combat_exp") * 2 /3 &&  !tar->query_condition("killer") ) 
                return notify_fail("人家比你强这么多，推火墙你必死无疑，还是别去送死了。\n");
...
if (me->query("combat_exp")<2000000) //经验少于2M不许PK，防止大米干扰人家JOB！
		return notify_fail("对不起,只有大于2M才可以火墙。\n");
...
if(userp(me) && me->query("env/damage"))             tell_object(me,WHT"你对"+ target->query("name") +"造成了"RED+me->query_skill("force")*2+ WHT"点攻击伤害。\n"NOR); 
if(userp(target)&& target->query("env/damage"))      tell_object(target,WHT""+ me->query("name") +"的攻击对你造成了"RED+me->query_skill("force")*2+ WHT"点伤害。\n"NOR); ;
	}
      	else {
