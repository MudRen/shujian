...
//            me->add("max_neili", - (1+random(3)));
             	...
             	else
             	{
                    ...
                    target->receive_damage("qi", (me->query_skill("strike")+me->query_skill("force")));
                    target->receive_wound("qi", (me->query_skill("strike")+me->query_skill("force"))/3);
...
if(userp(me) && me->query("env/damage"))             tell_object(me,WHT"你对"+ target->query("name") +"造成了"RED+me->query_skill("strike")+me->query_skill("force")+ WHT"点攻击伤害。\n"NOR); 
if(userp(target)&& target->query("env/damage"))      tell_object(target,WHT""+ me->query("name") +"的攻击对你造成了"RED+me->query_skill("strike")+me->query_skill("force")+ WHT"点伤害。\n"NOR); ;
                 	return 1;
               }
        }
        else
