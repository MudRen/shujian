...
int valid_learn(object me)
{
	if (me->query_temp("weapon") || me->query_temp("secondary_weapon"))
        ...
        if (me->query("str") < 26)
                return notify_fail("你先天膂力不足，无法学习淳厚的幻魔胧天舞。\n");

        if ((int)me->query_skill("force") < 150)
                return notify_fail("你的内功火候不够，无法练幻魔胧天舞。\n");

	if ((int)me->query("max_neili") < 1500)
		return notify_fail("你的内力太弱，无法练幻魔胧天舞。\n");

        if ((int)me->query_skill("strike", 1) < 100)
                return notify_fail("你的基本掌法火候不够，无法练幻魔胧天舞。\n");

	if ((int)me->query_skill("strike", 1) < (int)me->query_skill("guzhuo-zhang", 1))
		return notify_fail("你的基本掌法水平有限，无法领会更高深的幻魔胧天舞。\n");
...
int practice_skill(object me)
{
	...
	if ((int)me->query("neili") < 80)
		return notify_fail("你的内力不够练幻魔胧天舞。\n");
