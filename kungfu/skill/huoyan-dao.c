...
if (random(me->query_skill("strike", 1)) > 100 
			 && random(me->query_skill("huoyan-dao", 1)) > 100  && !random(3)
				...
				me->add("neili", -20);
				return ([
					"action": RED"$N将火焰刀气连连催动，直逼$n，$n只觉全身竟似被一阵热浪包围！"NOR,
...
if( !me->query_temp("hyd/auto") && userp(me) )
 ...
 if(  random(10)> 5 && random(me->query("combat_exp")) > target->query("combat_exp")*2/3 && me->query_skill("huoyan-dao",1) > 350) 
 {
        	message_vision(HBRED "$N佛法精湛，此刻做明王忿怒像，直如怒佛灭世，火焰翻腾，将$n逼得无路可退！" NOR, me,target);
        	target->start_busy(1+random(2));
                   me->set_temp("hyd/auto", 1);
                         me->add_temp("apply/damage",  me->query_skill("huoyandao", 1)/6);
