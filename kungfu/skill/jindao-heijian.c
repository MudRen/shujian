...
mapping query_action(object me, object weapon)
{
	...
	if (objectp(weapon)) {
	if (me->query_temp("jindao-heijian/luanren",1)&&!me->query_temp("jindao-heijian/doublt_attack",1)&& objectp(target = me->select_opponent())) {
		me->set_temp("jindao-heijian/doublt_attack",1);
		weapon_attack(me,target);
		me->delete_temp("jindao-heijian/doublt_attack");
	}
...
mixed hit_ob(object me, object victim, int damage_bonus)
{
		...
		&&!me->query_temp("jindao-heijian/nizhuan",1)
		&&!me->query_temp("jindao-heijian/luanren",1)
		&&!me->query_temp("jindao-heijian/doublt_attack",1)
...
void weapon_attack(object me, object victim)
{
	...
	if(me->query_temp("jindao-heijian/doublt_attack") && !me->query_temp("jindao-heijian/doublt_attack1")
	&& objectp(weapon2)) {
		me->set_temp("jindao-heijian/doublt_attack1", 1);
              weapon->unequip();
              weapon2->wield();
		COMBAT_D->do_attack(me, victim, weapon2, i);
              weapon2->unequip();
              weapon->wield();
		me->delete_temp("jindao-heijian/doublt_attack1");
	} else if (!me->query_temp("jindao-heijian/doublt_attack1")){
