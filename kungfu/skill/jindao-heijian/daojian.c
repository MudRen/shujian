...
int perform(object me, object target)
{
	...
	if (!objectp(weapon2)) return notify_fail("你只有一种兵器就想刀剑齐挥？\n");
        me->add_temp("apply/attack", me->query_skill("jindao-heijian",1)/10);
        message_vision(HIC"$N大喝一声立时武功招势奇幻无方，刀剑招势逆转，挥剑砍了三刀！\n"NOR, me, target);
	if (!me->query_temp("jindao-heijian/nizhuan",1)) {
		me->set_temp("jindao-heijian/nizhuan",1);
		i=4;
	}
	if (weapon->query("skill_type")=="blade") {
		weapon->unequip();
...
weapon->name()+weapon2->name()+HIR"势如飞凤，劈削不到。\n"NOR,me,target);
	if (!me->query_temp("jindao-heijian/nizhuan",1)) {
        ...
        me->add_temp("apply/attack", me->query_skill("jindao-heijian",1)/6);
	if (weapon->query("skill_type")=="blade") {
		COMBAT_D->do_attack(me, target, weapon,1);
		COMBAT_D->do_attack(me, target, weapon,1);
		COMBAT_D->do_attack(me, target, weapon,1);
