...
mapping query_action(object me, object weapon)
{
	...
	if(me->query_temp("tisha")){
		switch(me->query_temp("tisha")) {
			case 4: msg = HBBLU+HIW"$N直抢而前，脚下一踬，忽然一个筋斗摔了过去，双手已向$n胸口拍了过去"NOR; break;
			case 3: msg = HBBLU+HIC"$N坐倒在地，双拳猛捶自己胸膛，待$n靠近突然间脚踢飞沙，抽出$w"HBBLU+HIC"向$n的$l疾刺"NOR;break;
			case 2: msg = HBBLU+HIM"$n在霎时之间，但觉飞雪扑面，双眼不能见物，茫然不知所措，$N趁势用"YEL"肩部"HBBLU+HIM"从背后向$n撞去"NOR; break;
			case 1: msg = HBBLU+HIR"$N随着$n的去式，巧运"HIR"乾坤大挪移神功"HBBLU+HIR"，钻到$n的身前，将手中$w"HBBLU+HIR"向$n的$l插去"NOR;break;
			default: msg = HBBLU+HIW"$N直抢而前，脚下一踬，忽然一个筋斗摔了过去，双手已向$n胸口拍了过去"NOR; break;
		}  
		me->add_temp("tisha", -1);
		return ([
			"action": msg,
