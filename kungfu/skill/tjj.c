...
//	if( !me->query("wudang/jianjue") ) return 0;
...
mixed hit_ob(object me, object victim, int damage_bonus)
{
	...
	if( me->query("env/太极剑法") && weapon1 && i > 150 && me->query("jiali") && living(victim)
	 && me->query("max_neili") > 1200 && me->query("neili") > 1000 && random(10) >= 5
	 ...
	 && random(me->query("combat_exp")) > victim->query("combat_exp")/3 
		...
		switch(weapon->query("material")) {
			...
			case "paper":
					...
					if( me->query("wudang/jianjue") && me->query_temp("jianjue") < 1 && damage_bonus > 100
					 && !victim->is_busy() && !victim->query_temp("lost_attack") && random(3) ) {
						...
						if( random(i) > 250
						 && random(me->query_str()) > victim->query_con()/2 ) {
							...
							message_vision(HIB"$p大开大阖，势道雄浑，有如泰山压顶之势攻向$P要害！\n"NOR, victim, me);
						}
						else
						if( random(i) > 250
						 && random(me->query_int()) > victim->query_int()/3 ) {
							...
							message_vision(HIY"$P只见眼前突的剑法大变，忽而轻灵如山，忽而灵动如羽，不禁心中一动，惧意暗生！\n"NOR, victim, me);
						}
					}
