...
if(!me->query("quest/wdjiuyang/pass") && !me->query_temp("quest/wdjiuyang/pass") ) return 0;
        ...
        d = me->query("qi") - damage;
         	
        ap = ob->query_skill("force");
        dp = me->query_skill("force");

 if (random(dp)> ap*2 /3 && random(me->query("neili")) > ob->query("neili")/3 && lvl >450 )
        {
  ...
  else if (random(dp)> ap /2 && me->query("neili") > ob->query("neili")/2 )
        {
	          i = -damage;        //削减并反弹所有攻击。
           ob->receive_damage("qi", damage,me);
           ob->receive_wound("qi", damage/3,me);
  ...
  else if (random(dp+ap)> ap || (lvl>450 && d <= me->query("max_qi")*2/3 ) || lvl>550)
        {
