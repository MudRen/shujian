...
#include <ansi.h>
#include <localtime.h>
#include "/quest/gmd/houtu/sys/list.h"

#define MAX_ROOM_INDEX 20
#define OFFICE_NPC     "/quest/gmd/npc/office"
#define MIRROR_MEMORY  "/quest/gmd/houtu/sys/memory"
#define KILLER_NPC     "/quest/gmd/houtu/npc/killer"
#define BOSS_NPC       "/quest/gmd/houtu/npc/zhuang"
...
sscanf(mirror_maps[j],"/quest/gmd/houtu/%s",fileName);
			memory->set(fileName,maps);
			maps->set("memory",memory);
                 if(maps->query("short") == YEL"厚土旗"NOR);
			{
				start_room = maps;
			}
			index ++;
