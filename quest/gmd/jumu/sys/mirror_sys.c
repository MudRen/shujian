...
#include <ansi.h>
#include <localtime.h>
#include "/quest/gmd/jumu/sys/list.h"

#define MAX_ROOM_INDEX 20
#define OFFICE_NPC     "/quest/gmd/npc/office"
#define MIRROR_MEMORY  "/quest/gmd/jumu/sys/memory"
#define KILLER_NPC     "/quest/gmd/jumu/npc/killer"
#define BOSS_NPC       "/quest/gmd/jumu/npc/zhuang"
...
sscanf(mirror_maps[j],"/quest/gmd/jumu/%s",fileName);
			memory->set(fileName,maps);
			maps->set("memory",memory);
                 if(maps->query("short") == HIG"巨木旗"NOR);
			{
				start_room = maps;
			}
			index ++;
