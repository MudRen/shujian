...
#include <ansi.h>
#include <localtime.h>
#include "/quest/gmd/liehuo/sys/list.h"

#define MAX_ROOM_INDEX 20
#define OFFICE_NPC     "/quest/gmd/npc/office"
#define MIRROR_MEMORY  "/quest/gmd/liehuo/sys/memory"
#define KILLER_NPC     "/quest/gmd/liehuo/npc/killer"
#define BOSS_NPC       "/quest/gmd/liehuo/npc/zhuang"
...
sscanf(mirror_maps[j],"/quest/gmd/liehuo/%s",fileName);
			memory->set(fileName,maps);
			maps->set("memory",memory);
                 if(maps->query("short") == HIR"烈火旗"NOR);
			{
				start_room = maps;
			}
			index ++;
