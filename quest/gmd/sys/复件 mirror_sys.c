...
#include <ansi.h>
#include <localtime.h>
#include "/quest/gmd/sys/list.h"

#define MAX_ROOM_INDEX 20
#define OFFICE_NPC     "/quest/gmd/npc/office"
#define MIRROR_MEMORY  "/quest/gmd/sys/memory"
#define KILLER_NPC     "/quest/gmd/npc/killer"
#define BOSS_NPC       "/quest/gmd/npc/boss"
...
int mirror(object* player,object leader)
{
	...
	for(j=0;j<size;j++)
	{
		...
		if(maps)
		{
			...
			sscanf(mirror_maps[j],"/quest/gmd/%s",fileName);
			memory->set(fileName,maps);
			maps->set("memory",memory);
// 5:         set("short", HIY"总坛前厅"NOR)
                        if(maps->query("short") == HIY"总坛前厅"NOR);
			{
				start_room = maps;
			}
			index ++;
...
void mirror_ok(object me)
{
	...
	{
		me->move(start_room);
             // "quest/gmd/npc/kongxing"->move(start_room);
