...
mapping name;
	...
	set("title", HBRED+HIW+title[random(sizeof(title))]+NOR+(i?"":"女")+"匪首");
	set("gender", i?"男性":"女性");
	set("age", 27+random(30));
	set("long","这个匪首是"+query("title")+"来到黑风寨的大当家，看起来凶狠异常。\n");
...
void die()
{
	...
	for(i=0;i<size;i++)
	{
		...
		if(exp)
		{
			...
			tell_object(team[i],HIY"【剿匪黑风寨】恭喜你挑战镜像成功！请拾取物品，镜像将在1分钟后关闭。\n"NOR);
