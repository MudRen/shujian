...
void create()
{
	...
	set("title", HBRED+HIW+title[random(sizeof(title))]+NOR+(i?"":"女")+"匪寇");
	set("gender", i?"男性":"女性");
	set("age", 27+random(30));
	set("long","这个匪寇是"+query("title")+"的匪徒，看样子穷凶极恶。\n");
...
//tell_object(find_player("rock"),sprintf("%O %O %O\n",memory,me));
        if(!me)
        {
                tell_room(environment(ob),HIM"匪寇仓皇之下逃跑了。\n"NOR);
                destruct(ob);
                return;
        }	
	memory->add("make_boss",1);
	tell_room(environment(ob),HIR"匪寇慌乱之下逃跑了。\n"NOR);
