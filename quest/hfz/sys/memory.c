...
#define SYSTEAM "/quest/hfz/sys/mirror_sys"
...
*/
	...
	if(me->query("make_boss") >= 15 && !me->query("send_boss"))
	{
		for(i=0;i<size;i++)
		{
			str = userId[sprintf("%d",i)];
...
if(player->is_team_leader()) leader=player;
			tell_object(player,HIR"【剿匪黑风寨】黑风寨BOSS出现，请各位小心。\n"NOR);
			players += ({ player });
		}
                // SYSTEAM->make_killer(leader,1);
SYSTEAM->make_killer(players,1);
		me->set("send_boss",1);
//   remove_call_out("make_killer",2,player);   //停止刷怪
	}
	//已到副本生存期限
	if(me->query("mirror_time") <= time())
	{
		for(i=0;i<size;i++)
		{
			...
			if(!str || !stringp(str)) continue;
			player = find_player(str);
			tell_object(player,HIR"【剿匪黑风寨】镜像已经瓦解，欢迎再次挑战。\n"NOR);
		}
		if(me->delete_map())
		{
			destruct(me);
...
int delete_map()
{
	...
	for(i=0;i<size;i++)
	{
		...
		for(j=0;j<size2;j++)
		{
			...
			if(userp(objs[j]))
			{
				tell_object(objs[j],HIR"【剿匪黑风寨】你被传回安全区。\n"NOR);                        
                                objs[j]->delete_temp("quest/hfz");
...
varargs int len_exits(int rand)
{
	...
	//		tell_object(find_player("rock"),sprintf("DEBUG: i = %d\n",i));
			...
			for(j=0;j<size2;j++)
			{
				...
				sscanf(file,"/quest/hfz/%s",fileName);
