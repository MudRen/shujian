...
void create()
{
	set_name(HIR"洪荒·神圣勇者战袍"NOR, ({"yongzhe zhanpao"}) );
	set_weight(400);
	if(clonep())
...
return 1;
                      ...
                      if (me->query_temp("armor/cloth")->query("id") == "yongzhe zhanpao"
        && me->query_temp("armor/waist")->query("id") == "yongzhe huyao"
 &&  me->query_temp("armor/wrists")->query("id") == "yongzhe huwan"
 &&  me->query_temp("armor/surcoat")->query("id") == "yongzhe pifeng"
 &&  me->query_temp("armor/shield")->query("id") == "yongzhe dunpai"
 &&  me->query_temp("armor/hands")->query("id") == "yongzhe shoutao"
 &&  me->query_temp("armor/boots")->query("id") == "yongzhe zhanxue"
 &&  me->query_temp("armor/head")->query("id") == "yongzhe toukui")

       {
                msg = HIR"$n装备的惊人之处此时方显露出来，只见$N对$n的攻击只如泥牛入海，又似击在铜墙铁壁之上一般。\n"NOR;
// damage = me->query("jinglian/changqun")*200;     damage 还用赋值吗？系统会不会自动计算啊？对方打我的伤害？
...
if( me->query("jinglian/yongzhe zhanpao"))
{
j = me->query("jinglian/yongzhe zhanpao");
