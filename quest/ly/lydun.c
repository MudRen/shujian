...
void create()
{
	set_name(CYN"天琊·狼牙盾牌"NOR, ({"langya dunpai"}) );
	set_weight(400);
	if(clonep())
...
if( me->query("jinglian/langya dunpai"))
{
j = me->query("jinglian/langya dunpai");

             set("armor_prop/armor", j*30);
             set("armor_prop/parry", j*30);
             // set("armor_prop/dodge", j*30);
             // set("armor_prop/dexerity",j*3);
             // set("armor_prop/strength",j*3);
              // set("armor_prop/intelligence",j*3);
              // set("armor_prop/constitution",j*3);
