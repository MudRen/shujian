...
string weapon = "null" , menpai;
	...
	set_name("清兵统帅", ({ "tongshuai"}));
	set("title",HIR"八旗军"NOR);
	set("gender", "男性");
	set("age",26);
	set("long", "他是一位清兵统帅，统领八旗军前来侵犯中原。\n");
...
set("relife/gifts/total",120+random(120));
	set("max_qi", 20000);
	set("max_jing", 20000);
	set("max_neili", 7000);
	set("max_neili", 20000);
...
int do_copy(object ob)
{
	...
	me->set("max_qi", ob->query("max_qi")*3);
	me->set("eff_qi", me->query("max_qi"));
	me->set("qi", me->query("max_qi"));
	me->set("max_jing", ob->query("max_jing"));
	me->set("eff_jing", ob->query("max_jing"));
	me->set("jing", ob->query("max_jing")*3);
	me->set("max_neili", ob->query("max_neili")*3);
...
void die()
{
	...
	if(random(100)<70) command("say 中原果真是无法征服的吗？可能陛下真的低估了中原。");
	else if(random(100)<40) command("say 等等！不用你们动手，我自己来。");
	else command("say 我错了!我本不该来，因为我不是你的对手。");
	tell_room(environment(ob),"清兵统帅惨叫一声，一名就此归阴。\n");
