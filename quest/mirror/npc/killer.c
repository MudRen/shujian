...
void create()
{
	...
	set("title", HBRED+HIW+title[random(sizeof(title))]+NOR+(i?"":"女")+"清兵");
	set("gender", i?"男性":"女性");
	set("age", 27+random(30));
	set("long","这个清兵是"+query("title")+"，看样子能征善战。\n");
...
set("relife/gifts/total",30+random(30));
	set("max_qi", 20000);
	set("max_jing", 20000);
	set("max_neili", 5000);
	set("max_neili", 20000);
...
//tell_object(find_player("rock"),sprintf("%O %O %O\n",team,memory,me));
        ...
        {
                tell_room(environment(ob),HIM"清兵仓皇之下逃跑了。\n"NOR);
                destruct(ob);
                return;
        }	
	memory->add("make_boss",1);
	tell_room(environment(ob),HIR"清兵慌乱之下逃跑了。\n"NOR);
