...
#define SYSTEAM "/quest/mirror/sys/mirror_sys"
...
varargs void heart(int open)
{
	...
	if(me->query("open_time")+600 <= time() && !me->query("fanghuo")) //5分钟后
	{
		for(i=0;i<size;i++)
		{
			...
			player = find_player(str);
			tell_object(player,HIY"【号令天下】清兵放火了，整条街都在燃烧。\n"NOR);
		}
		me->len_exits(1);
		me->set("fanghuo",1);
	}
	...
	else if(me->query("open_time")+1200 <= time())  //10分钟后
	{
		if(!me->query("rand_2"))
		{
			for(i=0;i<size;i++)
			{
				...
				if(!str || !stringp(str)) continue;
				player = find_player(str);
				tell_object(player,HIY"【号令天下】清兵摧毁了山海关。\n"NOR);
			}
			me->len_exits(3);
			me->set("rand_2",1);
		}
	...
	if(me->query("make_boss") >= 150 && !me->query("send_boss"))
	{
		for(i=0;i<size;i++)
		{
			...
			if(!str || !stringp(str)) continue;
			player = find_player(str);
			tell_object(player,HIY"【号令天下】清兵统帅出现，请各位小心。\n"NOR);
			players += ({ player });
		}
...
varargs int len_exits(int rand)
{
	...
	//		tell_object(find_player("rock"),sprintf("DEBUG: i = %d\n",i));
			...
			for(j=0;j<size2;j++)
			{
				...
				sscanf(file,"/quest/mirror/%s",fileName);
