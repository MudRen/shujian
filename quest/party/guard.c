...
void create()
{
	...
	name = RNAME_D->get_random_name(i);
        ...
        set_name(name["name"], name["id"] + ({ "jianxi" }));
...
command("say 哼，居然连" + me->name(1) + "也来凑热闹！");
destruct(ob);
                return;
        }
else {
me->delete_temp("party_job_start");
me->set_temp("party_job_over",1);
}
	::die();
}
...
int do_copy(object target,int mode)
{
	...
	if(mode) {
		t = 12+random(4);
		set("long",query("long")+"最近听说各门各派都混进了不少奸细，这就是其中一个！\n");
