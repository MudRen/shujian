...
#define SYSTEAM "/quest/xyfm/sys/mirror_sys"
...
varargs void heart(int open)
{
	...
	if(me->query("open_time")+600 <= time() && !me->query("fanghuo")) //5分钟后
	{
		for(i=0;i<size;i++)
		{
			...
			player = find_player(str);
			tell_object(player,HIY"【降妖伏魔】起火了，整条街都在燃烧。\n"NOR);
		}
		me->len_exits(1);
		me->set("fanghuo",1);
	}
	else if(me->query("open_time")+900 <= time() && !me->query("rand"))  //10分钟后
	{
		for(i=0;i<size;i++)
		{
			...
			if(!str || !stringp(str)) continue;
			player = find_player(str);
			tell_object(player,HIY"【降妖伏魔】整条街燃烧殆尽，已经分不清方向了。\n"NOR);
		}
		me->len_exits(2);
		me->set("rand",1);
	}
	else if(me->query("open_time")+1200 <= time())  //10分钟后
	{
		if(!me->query("rand_2"))
		{
			for(i=0;i<size;i++)
			{
				...
				if(!str || !stringp(str)) continue;
				player = find_player(str);
				tell_object(player,HIY"【降妖伏魔】怨孽摧毁了整个地带。\n"NOR);
			}
			me->len_exits(3);
			me->set("rand_2",1);
		}
	...
	if(me->query("make_boss") >= 150 && !me->query("send_boss"))
	{
		for(i=0;i<size;i++)
		{
			...
			if(!str || !stringp(str)) continue;
			player = find_player(str);
			tell_object(player,HIY"【降妖伏魔】怨孽头目出现，请小心。\n"NOR);
			players += ({ player });
		}
	...
	if(me->query("mirror_time") <= time())
	{
		for(i=0;i<size;i++)
		{
			...
			if(!str || !stringp(str)) continue;
			player = find_player(str);
			tell_object(player,HIY"【降妖伏魔】镜像已经瓦解，欢迎再次挑战。\n"NOR);
		}
		if(me->delete_map())
		{
			destruct(me);
...
int delete_map()
{
	...
	for(i=0;i<size;i++)
	{
		...
		for(j=0;j<size2;j++)
		{
			...
			if(userp(objs[j]))
			{
				tell_object(objs[j],HIY"【降妖伏魔】你被传回安全区。\n"NOR);
...
varargs int len_exits(int rand)
{
	...
	//		tell_object(find_player("rock"),sprintf("DEBUG: i = %d\n",i));
			...
			for(j=0;j<size2;j++)
			{
				...
				sscanf(file,"/quest/xyfm/%s",fileName);
