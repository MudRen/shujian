...
mixed ob_hit(object ob, object me, int damage)
{
	 int ap, dp;
        ...
        if (random(dp+ap)> ap && me->query_skill("wudang-jiuyang",1) > 100 && me->query_skill("wudang-jiuyang",1) < 151 )
        {
	        i = - damage/3 - random(damage*2/3);  //削减一小半至全部的攻击
                                
              message_vision(HIC "\n$N将氤氲紫气运遍全身，顷刻间使遍于四肢百骸，几至全身无缺陷处，无凹凸处，无断续处。"
                                   "\n随人所动，随屈就伸，以心使劲，中招之处竟不觉如何疼痛。\n"NOR,me,ob);
         return i;
         }                          
      else if (random(dp+ap)> ap && me->query_skill("wudang-jiuyang",1) > 151 && me->query_skill("wudang-jiuyang",1) < 181 )
        {
	      i = -damage;        //削减并反弹所有攻击。
          ob->receive_damage("qi", damage,me);
                ob->receive_wound("qi", damage/3,me);

              message_vision(HIC "\n$N以四两拨千斤之法，我劲接彼劲，曲中求直，借力打人，体内真气流转，敌弱便弱，敌强愈强。"
                                   "\n那掌力击在他身上，尽数反弹了出来，竟是丝毫没有受到伤害。\n"NOR,me,ob);
              return -i;                               
           }      

 if (random(dp+ap )> ap && me->query_skill("wudang-jiuyang",1) > 181 && me->query_skill("wudang-jiuyang",1) < 201 )
        {
	        i = -damage;
ob->receive_damage("qi", damage,me);
                ob->receive_wound("qi", damage/3, me);

              message_vision(HIC "\n$N已经澈晓道家冲虚圆通之道，又将生平所学与九阳真经相对照，武当武功几臻登峰造极之境，"
                                  "\n当下用意不用劲，前后左右，全无定向，反守为攻，竟是后发制人，先发者制于人。\n"NOR,me,ob);
              COMBAT_D->do_attack(me, ob, ob->query_temp("weapon"), userp(ob)?1:3);
