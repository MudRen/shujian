...
//                                                if (userp(me)) me->improve_skill(flag?"parry":attack_skill, 1);
	...
	if( damage > 0 ) {
		...
		if( victim->is_busy() ) {
              ...
              if(!random(5)) victim->start_busy(-1);
		...
		if (objectp(killer)) {
			...
			killer->set("pk_time", time());
                ...
                if (wanted_time
                    && userp(killer)
                    && (
                            killer->query_temp("kill_other/"+victim->query("id"))
			|| !victim->query_temp("kill_other/"+killer->query("id"))
				...
				if (userp(killer)) {
					...
					&& !killer->query("revenge/"+victim->query("id"))
                                         && !muderer->query("revenge/"+victim->query("id")) {
						if (multi)
...
----------------------------------------
else if (killer->query("revenge/"+victim->query("id"))) {
...
else if (muderer->query("revenge/"+victim->query("id")) && !killer->query("revenge/"+victim->query("id")) ) {
                                                 
						str = sprintf("被%s复仇%s死了。", muderer->name(), vmark);
						if (multi) {
							muderer->add("PKS", -1);	// 帮忙复仇不算 PK
							muderer->set("pk_time", muderer->query("last_pk_time"));
						}
